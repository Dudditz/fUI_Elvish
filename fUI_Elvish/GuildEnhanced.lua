...
local activezone, inactivezone = {r=0.4, g=0.78, b=1}, {r=0.65, g=0.65, b=0.65}
...
local function OnEnter(_, _, noUpdate)
	...
	if guildMotD ~= '' then
		DT.tooltip:AddLine(' ')
		DT.tooltip:AddLine(format(guildMotDString, GUILD_MOTD, guildMotD), tthead.r,tthead.g,tthead.b, 1)
	end
	...
	for i, info in ipairs(guildTable) do
		...
		local shortName = strmatch(info.name,'([^%-]+).*')

		local zoneColor = format('|cff%02x%02x%02x', zonec.r*255, zonec.g*255, zonec.b*255)

		local leftText = zoneColor .. info.zone .. '|r'

		if info.note ~= '' then
			leftText = leftText .. ' - ' .. info.note
		end

		if info.officerNote ~= '' then
			leftText = leftText .. ' |cffff8800(' .. info.officerNote .. ')|r'
		end

		local rightText = format(
			'|cff%02x%02x%02x%d|r |cff%02x%02x%02x%s|r - %s',
			levelc.r*255, levelc.g*255, levelc.b*255, info.level,
			classc.r*255, classc.g*255, classc.b*255, shortName,
			info.rank
		)

		DT.tooltip:AddDoubleLine(rightText, leftText, 1,1,1, 1,1,1)
	end
...
--[[G.datatexts.settings.GuildEnhanced = {
    NoLabel = false,
    Label = "Guild",
}]]

local DT = E:GetModule('DataTexts')
local oldDT = DT.RegisteredDataTexts["Guild"]

-- Remove old datatext entirely
if oldDT then
    DT.RegisteredDataTexts["Guild"] = nil
end

--DT:RegisterDatatext('GuildEnhanced', 'Social', { 'CHAT_MSG_SYSTEM', 'GUILD_ROSTER_UPDATE', 'PLAYER_GUILD_UPDATE', 'GUILD_MOTD', 'MODIFIER_STATE_CHANGED' }, OnEvent, nil, Click, OnEnter, nil, 'Guild Enhanced', nil, ApplySettings)
