...
-- create a popup
E.PopupDialogs.PP_SET_BN_BROADCAST = {
					...
					if info.isOnline then
						...
						if not shouldSkip then
							...
							if info.client and info.client == wowString then
								...
								-- fallback for missing faction
								local faction = info.factionName or info.faction -- some tables use info.faction
								if faction == "Horde" then
									factionIcon = "|cffff3333H|r"
								elseif faction == "Alliance" then
									factionIcon = "|cff3399ffA|r"
								else
									factionIcon = "?" -- unknown / non-WoW friend
								end

								-- AFK / DND marker
								local status = ""
								if info.isAFK then
									status = " |cffaaaa00<AFK>|r"
								elseif info.isDND then
									status = " |cffff5555<DND>|r"
								end

								-- group icon if in party/raid
								local groupIcon = InGroup(info.characterName) or ""

								-- left side: LEVEL (colored) + CharacterName (class colored) + BattleTag + status + group icon
								local left = format(
									'|cff%02x%02x%02x%d|r |cff%02x%02x%02x%s|r (%s)%s %s',
									levelc.r*255, levelc.g*255, levelc.b*255,   -- level color
									info.level,
									classc.r*255, classc.g*255, classc.b*255,   -- character name color
									info.characterName,
									info.accountName,   -- BattleTag
									status,
									groupIcon
								)

								-- right side: [Faction] Zone - Server (zone colored)
								local zoneColor
								local cityZones = {
									...
									["Valdrakken"]=true, ["Shattrath City"]=true, ["Dalaran"]=true, ["Dazar'alor"]=true,
									["Boralus"]=true, ["Dornogal"]=true
								}

								if cityZones[info.zoneName] then
									-- major cities = light blue
									zoneColor = {r = 0.4, g = 0.78, b = 1}
								else
									-- color by friend level relative to player
									zoneColor = GetQuestDifficultyColor(info.level or 1)
								end

								local right = format(
									'[%s] |cff%02x%02x%02x%s|r - %s',
									factionIcon,
									zoneColor.r*255, zoneColor.g*255, zoneColor.b*255,
									info.zoneName or UNKNOWN,
									info.realmName or UNKNOWN
								)

								-- add to tooltip
								TooltipAddXLine(true, header, left, right, 238, 238, 238, 238, 238, 238)
								
								
							    else
								TooltipAddXLine(true, header, info.characterName..status, info.accountName, tthead.r,tthead.g,tthead.b, .9, .9, .9)
...
local function OnEvent(panel, event, arg1)
	...
	if db.NoLabel then
		...
		panel.text:SetFormattedText(displayString, db.Label ~= '' and db.Label or 'FRIENDS'..': ', onlineFriends + numBNetOnline)
	end
end
...
--[[G.datatexts.settings.FriendsEnhanced = {
    NoLabel = false,
    Label = "FRIENDS",
	showAllFriends = true,
    hideAFK = false,
...
--DT:RegisterDatatext('FriendsEnhanced', 'Social', { 'BN_FRIEND_ACCOUNT_ONLINE', 'BN_FRIEND_ACCOUNT_OFFLINE', 'BN_FRIEND_INFO_CHANGED', 'FRIENDLIST_UPDATE', 'CHAT_MSG_SYSTEM', 'MODIFIER_STATE_CHANGED' }, OnEvent, nil, Click, OnEnter, nil, "Friends Enhanced", nil, ApplySettings)
